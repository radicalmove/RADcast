...
If the default optimized model appears unavailable, check that the active environment can import both `resemble_enhance` and `nara_wpe`. On Apple Silicon helpers, the bootstrap path now installs current `torch`/`torchaudio`/`torchvision`/`torchcodec` wheels and the optimized path prefers `mps` for the Resemble stage when available. `RADcast Optimized` now also runs in-process inside the long-lived helper/server process, so repeated jobs reuse the loaded `Resemble` model instead of paying a fresh CLI cold start every time.
